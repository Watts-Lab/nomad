...
ALLOWED_MIN_PTS = {2, 3, 4} # restricted min pts

PARAM_RANGES = { #allowed param changes
    "dbstop": {
        "dist_thresh": (1, 100), # placeholder range --> need real values chosen from aggregated metrics plots
        "roam_dist": (1, 100), # placeholder range
        "min_pts": ALLOWED_MIN_PTS,
    },
    "lachesis": {
        "roam_dist": (1, 100),
        "min_pts": ALLOWED_MIN_PTS,
...
# validate out of range inputs
def validate_cached_params(algorithm, **params):
    """Validate dashboard/cache parameters against allowed ranges."""
    if algorithm not in PARAM_RANGES:
        raise ValueError(f"Unknown algorithm: {algorithm}")

    allowed = PARAM_RANGES[algorithm]
    ...
    for name, value in params.items():
        ...
        if name == "min_pts":
            if value not in allowed["min_pts"]:
                raise ValueError(
                    "Parameter out of range. "
                    f"Please choose min_pts in {sorted(allowed['min_pts'])}."
                )
        else:
            ...
            if value < low or value > high:
                raise ValueError(
                    "Parameter out of range. "
                    f"Please choose {name} between {low} and {high}."
                )

# cached filename naming convention         
def cached_result_name(algorithm, **params):
    """Create deterministic cached-result filename."""
    parts = [algorithm]

    if "dist_thresh" in params:
        parts.append(f"dist-{params['dist_thresh']}")

    if "roam_dist" in params:
        parts.append(f"roam-{params['roam_dist']}")

    if "min_pts" in params:
        parts.append(f"minpts-{params['min_pts']}")
