...
def test_connect_building_door_success():
    ...
    for coord in corridor_blocks:
        city.blocks_gdf.loc[coord, 'kind'] = None
    ...
    assert city.blocks_gdf.loc[(5, 3), 'kind'] == 'street'
    assert city.blocks_gdf.loc[(5, 4), 'kind'] == 'street'
    assert city.blocks_gdf.loc[(5, 5), 'kind'] == 'street'
...
def test_connect_building_door_path_too_long():
    ...
    for y in range(1, 8):
        city.blocks_gdf.loc[(7, y), 'kind'] = None
    ...
    for x in range(3, 18):
        for y in range(3, 18):
            if not (x == 10 and y == 10):
                city.blocks_gdf.loc[(x, y), 'kind'] = 'building'
